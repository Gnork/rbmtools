...
public class RBMOriginalCorrect implements IRBM {
    ...
    public RBMOriginalCorrect(int numVisbible, int numHidden, float learningRate, float[][] weights) {
	...
	public RBMOriginalCorrect(int numVisbible, int numHidden, float learningRate) {
		this.numHidden = numHidden;
		this.numVisible = numVisbible;
		this.numHiddenWithBias = numHidden + 1;
		this.numVisibleWithBias = numVisbible + 1;
...
//		    		weights[r][c] += (posAssociations[r][c] - negAssociations[r][c]);
		    ...
		    for(int r = 0; r < negVisibleProbs.length; r++) {
		    	for(int c = 0; c < negVisibleProbs[0].length; c++) {
                            float tmp = dataWithBias[r][c] - negVisibleProbs[r][c];
